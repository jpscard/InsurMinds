...
def render(df):
    ...
    """
    st.header("📊 Painel de Controle e Métricas de Negócio")
    st.write("Análise dos principais indicadores e métricas extraídas do conjunto de dados.")
    ...
    with st.expander("⚙️ Configurar Mapeamento de Colunas Essenciais", expanded=True):
        st.info("Indique as colunas que correspondem aos conceitos de negócio para calibrar os gráficos automáticos.")
    ...
    with kpi2:
        ...
        st.metric(label="📦 Itens / Quantidade Total", value=valor_itens)
    with kpi3:
    ...
    # --- 3. GRÁFICOS CURADOS ---
    st.subheader("Análises Visuais Relevantes")
    ...
    with col_a:
        if coluna_cliente and coluna_valor:
            ...
            fig_clientes = px.bar(
                ...
                title="🏆 Top 10 Clientes por Valor Total", labels={coluna_valor: 'Valor Total (R$)', 'y': 'Cliente'}, text_auto='.2s'
            )
    ...
    with col_b:
        if coluna_produto and coluna_valor:
            ...
            fig_produtos = px.bar(
                ...
                title="🛍️ Top 10 Produtos por Faturamento", labels={coluna_valor: 'Valor Total (R$)', 'y': 'Produto'}, text_auto='.2s'
            )
    ...
    if coluna_data and coluna_valor and pd.api.types.is_datetime64_any_dtype(df[coluna_data]):
        ...
        if not df_tempo.empty:
            ...
            fig_tempo = px.line(
                ...
                title="📈 Faturamento Diário ao Longo do Tempo", labels={coluna_data: 'Data', coluna_valor: 'Faturamento (R$)'}, markers=True
            )
            ...
            if st.button("📌 Adicionar Gráfico de Tempo ao Relatório", key="pin_tempo"):
                item = {
                    "type": "chart", 
                    "category": "dashboard", 
                    "title": "Gráfico: Vendas no Tempo", 
                    "content": {"titulo": "Faturamento Diário ao Longo do Tempo", "dados": vendas_no_tempo, "metrica": "Faturamento (R$)", "fig": fig_tempo}
                }
                st.session_state.report_items.append(item)
    ...
    with st.expander("🔬 Análise Detalhada e Personalizada (Deep Dive)"):
        st.write("Cruze qualquer dimensão e métrica existente nos dados.")
